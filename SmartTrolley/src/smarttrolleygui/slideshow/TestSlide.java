...
public class TestSlide {
	...
	public void setUp() {
		...
		visualTesting = new TestSlideVisual();

		Thread JFXApplThread;
		JFXApplThread = new Thread("New JFXAppl") {
			public void run() {
				SmartTrolleyToolBox.print("JFXAppl thread");
				Application.launch(visualTesting.getClass(), (java.lang.String[]) null);
			}
		};
		JFXApplThread.start();

		// Pause to allow the application to launch
		while (TestSlideVisual.stage == null);

		// Now launch the instance of TestSlideVisual, which takes over the
		// displayed stage
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				try {
					visualTesting.start(TestSlideVisual.stage);
				} catch (Exception e) {
					SmartTrolleyToolBox.print("Could not load application");
				}
			}
		});
		...
		SmartTrolleyToolBox.delay(2000);

		productSlide = new Slide(xScaler, yScaler, graphicsList, imageList, audioList, textList, videoList, slideDuration);
	...
	*/
	public void setupImage() {
		SlideImage image = new SlideImage(imageURL, xImageStart, yImageStart, imageWidth, imageHeight, imageStartTime, 3,1,1);

		SlideImage image2 = new SlideImage(imageURL, xImageStart2, yImageStart2, imageWidth / 2, imageHeight / 2, imageStartTime, imageDuration,1,1);
	...
	*/
	public void setupGraphics() {
		...
		SlideShapeFactory shapeFactory = new SlideShapeFactory(points, graphicsHeight, graphicsWidth, graphicsFillColour, graphicsLineColour, graphicsStartTime, graphicsDuration,1,1);
	...
	*/
	public void setupVideo() {
		SlideVideo video = new SlideVideo(vidURL, xVidStart, yVidStart, vidWidth, vidHeight, vidLoop, vidStartTime, vidDuration,1);
	...
	*/
	...
	public void audioHasStoppedPlayingTest() {
		SmartTrolleyToolBox.delay(6000);
