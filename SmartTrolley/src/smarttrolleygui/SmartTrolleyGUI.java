...
public class SmartTrolleyGUI extends Application {
	...
	public Stage stage;
	final double MIN_WINDOW_WIDTH = 200.0;
	final double MIN_WINDOW_HEIGHT = 200.0;
	...
	public void start(Stage primaryStage) {
		try {
			...
			// for debugging purposes uncomment ONE of the following four lines
	...
	public void goToStartScreen() {
		try {
			 ...
			 * 
			 * // 2D array of Buttons with value of 5,5 Button[][] btn = new
			 * Button[4][4];
			 * 
			 * //Column is a vertical line and row is a horizontal line //Two
			 * FOR loops used for creating 2D array of buttons with values i,j
			 * for(int i=0; i<btn.length; i++){ for(int j=0; j<btn.length;j++){
			 * 
			 * //Initialising 2D buttons with values i,j btn[i][j] = new
			 * Button(""+i+","+""+j); btn[i][j].setPrefSize(20, 20);
			 * gridPane.add(btn[i][j], i, j); } } System.out.println("Hi");
			 * 
			 * //Adding GridPane to the scene
			 * 
			 * Scene scene = new Scene(gridPane); stage.setScene(scene);
			 * stage.show();
			 */
	...
	/**
	 * goToHomeScreen is called when the 'Home' button is pressed. It loads the
	 * screen which allows the user to browse through different product
	...
	public Initializable replaceSceneContent(String fxml) throws Exception {
		...
		InputStream inputStream = SmartTrolleyGUI.class
		...
		//button.relocate(MIN_WINDOW_WIDTH, MIN_WINDOW_HEIGHT*0.5);
